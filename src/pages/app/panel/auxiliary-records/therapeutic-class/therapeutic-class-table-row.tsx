import { PenLine, Search, Trash } from "lucide-react";
import { useState } from "react";

import { Button } from "@/components/ui/button";
import { Dialog, DialogTrigger } from "@/components/ui/dialog";
import { TableCell, TableRow } from "@/components/ui/table";

import { UpdateTherapeuticClassDialog } from "./update-therapeutic-class-dialog";
import { AlertDialog, AlertDialogTrigger } from "@/components/ui/alert-dialog";
import { AlertDialogConfirm } from "@/components/dialog/alert-confirm-content";
import { useAuth } from "@/contexts/authContext";
import { useMutation } from "@tanstack/react-query";
import {
  deleteTherapeuticClass,
  type DeleteTherapeuticClassBody,
} from "@/api/pharma/auxiliary-records/therapeutic-class/delete-therapeutic-class";
import { queryClient } from "@/lib/react-query";
import { toast } from "@/hooks/use-toast";
import { handleApiError } from "@/lib/utils/handle-api-error";

export interface TherapeuticClassTableRowProps {
  therapeuticClass: {
    id: string;
    name: string;
    description: string;
  };
}

export function TherapeuticClassTableRow({
  therapeuticClass,
}: TherapeuticClassTableRowProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);
  const { token } = useAuth();

  const { mutateAsync: deleteTherapeuticClassFn, isPending } = useMutation({
    mutationFn: (data: DeleteTherapeuticClassBody) =>
      deleteTherapeuticClass(data, token ?? ""),
    onSuccess() {
      queryClient.invalidateQueries({ queryKey: ["therapeutic-classes"] });
      toast({ title: "Classe terapêutica excluída com sucesso!" });
      setIsDeleteOpen(false);
    },
    onError(error) {
      toast({
        title: "Erro ao excluir classe terapêutica",
        description: handleApiError(error),
        variant: "destructive",
      });
    },
  });
  const handleDelete = () => {
    deleteTherapeuticClassFn({ id: therapeuticClass.id });
  };
  return (
    <TableRow>
      <TableCell>
        <Dialog>
          <DialogTrigger asChild>
            <Button variant={"outline"} size={"xs"}>
              <Search className="h-3 w-3" />
              <span className="sr-only">Detalhes do pedido</span>
            </Button>
          </DialogTrigger>
          {/* <OrderDetails /> */}
        </Dialog>
      </TableCell>
      <TableCell className="font-mono text-xs font-medium">
        {therapeuticClass.name}
      </TableCell>

      <TableCell>{therapeuticClass.description}</TableCell>

      <TableCell>
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          <DialogTrigger asChild>
            <Button variant={"outline"} size={"xs"}>
              <PenLine className="h-3 w-3" />
            </Button>
          </DialogTrigger>
          <UpdateTherapeuticClassDialog
            therapeuticClassId={therapeuticClass.id}
            open={isOpen}
            onSuccess={() => setIsOpen(false)}
          />
        </Dialog>
      </TableCell>
      <TableCell>
        <AlertDialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
          <AlertDialogTrigger asChild>
            <Button variant="outline" size="xs">
              <Trash className="h-3 w-3" />
            </Button>
          </AlertDialogTrigger>
          <AlertDialogConfirm
            title="Deseja excluir esta patologia?"
            description="Esta ação não pode ser desfeita."
            isPending={isPending}
            variant="destructive"
            cancelLabel="Cancelar"
            confirmLabel="Excluir"
            onConfirm={handleDelete}
          />
        </AlertDialog>
      </TableCell>
    </TableRow>
  );
}
